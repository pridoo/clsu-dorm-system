...
export default function ManagerDashboard() {
  ...
  useEffect(() => {
    const unsubAuth = onAuthStateChanged(auth, async (user) => {
      if (user) {
        try {
          // 1. Kunin muna ang Active System Configuration (Term Context)
          const configSnap = await getDoc(doc(db, "system", "config"));
          if (!configSnap.exists()) return;
          const activeConfig = configSnap.data();

          // 2. Kunin ang profile ng Manager
          ...
          if (userDoc.exists()) {
            ...
            // 3. STRICT QUERY: Dapat Building match + Not Archived + Active Term labels match
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-white border border-slate-300 shadow-[0_0_50px_rgba(0,0,0,0.08)] rounded-[45px] relative leading-none">
        ...
        <nav className="h-24 px-10 flex items-center justify-between border-b border-slate-200 bg-white/80 backdrop-blur-md relative z-20 flex-shrink-0 text-left leading-none">
          <div className="space-y-0.5 text-left leading-none">
            ...
            <h1 className="text-3xl font-extralight text-slate-900 tracking-tighter leading-none italic text-left leading-none">
          ...
          <div className="flex items-center gap-6 leading-none">
            <div className="flex flex-col items-end mr-2 text-right leading-none">
               <div className="flex items-center gap-2 mb-1 leading-none">
                  ...
                  <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest italic leading-none">Live: Sync Active</p>
               </div>
            ...
            <div className="flex items-center gap-3 bg-emerald-950 p-2 pr-6 rounded-[22px] shadow-2xl border border-emerald-800 leading-none">
              ...
              <div className="text-left leading-none">
                ...
                <p className="text-[12px] font-bold text-white tracking-tight italic uppercase leading-none">{managerInfo.name}</p>
              </div>
            </div>
          </div>
        </nav>

        <div className="px-10 py-8 grid grid-cols-4 gap-6 bg-[#f8fafc]/30 relative z-10 flex-shrink-0 leading-none">
          <ManagerStatCard label="Term Residents" value={stats.total.toString()} sub="Current Term" icon={<Users size={18}/>} color="emerald" glow="shadow-emerald-200/50" />
          ...
          <ManagerStatCard label="Pending" value={stats.unassigned.toString()} sub="No Room yet" icon={<UserMinus size={18}/>} color="amber" glow="shadow-amber-200/50" />
          ...
          <motion.div whileHover={{ y: -6, scale: 1.01 }} className="bg-emerald-950 rounded-[35px] p-6 text-white relative overflow-hidden group shadow-2xl shadow-emerald-900/40 border border-emerald-800 text-left leading-none">
             <div className="relative z-10 h-full flex flex-col justify-between">
               <div>
                  <p className="text-[9px] font-black text-emerald-400 uppercase tracking-widest mb-1 italic leading-none">Cluster Load</p>
        ...
        <div className="flex-1 px-10 pb-10 bg-[#f8fafc]/30 flex flex-col relative z-10 overflow-hidden text-left leading-none">
          <div className="flex items-center justify-between mb-6 px-1 leading-none">
             ...
             <div className="flex items-center gap-3 bg-white px-4 py-2 rounded-xl border border-slate-300 shadow-sm focus-within:border-emerald-500 transition-all w-64 text-left leading-none">
                ...
                <input 
                  ...
                  placeholder="Search current profiles..." 
          ...
          <div className="flex-1 bg-white border border-slate-200 rounded-[35px] shadow-[0_20px_60px_rgba(0,0,0,0.04)] flex flex-col overflow-hidden text-left leading-none">
            <div className="flex-1 overflow-y-auto scrollbar-hide text-left leading-none"> 
              <table className="w-full text-left border-separate border-spacing-0 leading-none">
                <thead className="sticky top-0 bg-white/95 backdrop-blur-md z-10 leading-none">
                  <tr>
                    <th className="px-10 py-6 text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 italic leading-none text-left">Resident Identity</th>
                    <th className="px-6 py-6 text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 text-center italic leading-none">Room Status</th>
                    <th className="px-10 py-6 text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 text-right italic leading-none">Registry Sync</th>
                  </tr>
                </thead>
                  ...
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={3} className="py-24 text-center leading-none border-none">
                        <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest italic leading-none">No active records for this term</p>
                      </td>
                    </tr>
                  ) : (
                    filteredResidents.map((student, i) => (
                      <tr key={i} className="group hover:bg-emerald-50/40 transition-all cursor-default leading-none">
                        <td className="px-10 py-5">
                          <div className="flex items-center gap-5 text-left leading-none">
                              ...
                              <div className="text-left leading-none">
                                <p className="text-[15px] font-bold text-slate-800 tracking-tight leading-none mb-1.5 uppercase italic leading-none">{student.first_name} {student.last_name}</p>
                                <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest italic font-sans leading-none">{student.student_id} • {student.course}</p>
                              </div>
                          </div>
                        </td>
                        <td className="px-6 py-5 text-center leading-none">
                            <span className={`text-[11px] font-black italic px-5 py-2 rounded-2xl border leading-none ${student.room === "NONE" ? 'bg-rose-50 border-rose-100 text-rose-400' : 'bg-slate-50 border-slate-200 text-slate-700'} shadow-sm`}>
  ...
  return (
    <motion.div whileHover={{ y: -6, scale: 1.01 }} className={`bg-white border-2 border-slate-200 p-6 rounded-[35px] transition-all text-left group relative shadow-md ${glow} leading-none`}>
      ...
      <div className="flex items-baseline gap-2 leading-none">
         ...
         <span className="text-[10px] font-bold text-slate-300 lowercase leading-none">{sub}</span>
      </div>
    </motion.div>
  );
}