...
import { collection, query, where, onSnapshot, doc, getDoc, writeBatch, serverTimestamp, getDocs } from 'firebase/firestore';
...
export default function RoomsPage() {
  ...
  // 1. Live Data Synchronization with Grouping Logic
  useEffect(() => {
        ...
        const roomsList = await Promise.all(roomSnap.docs.map(async (rDoc) => {
          ...
          const occupantsWithNames = await Promise.all(occupantsInRoom.map(async (occ) => {
             ...
             return {
               ...
               course: resData?.course || "N/A",
               year: resData?.year_level || "N/A",
               allocationId: occ.studentID 
             };
          }));
          ...
          return {
            ...
            floor: parseInt(room.room_number) <= 10 ? '1st Floor' : 'Upper Floor',
            total: room.total_beds,
            occupants: occupantsWithNames,
          };
        }));

        setRooms(roomsList.sort((a: any, b: any) => {
  ...
  const handleExecuteTransfer = async () => {
    ...
    try {
      ...
      const allocRef = doc(db, "allocations", movingResident.id);
  ...
  return (
    <div className="flex h-screen w-screen bg-[#f1f5f9] p-6 gap-6 overflow-hidden text-left font-sans text-slate-900">
      ...
      <LayoutGroup>
        <main className="flex-1 flex flex-col overflow-hidden bg-white border border-slate-300 shadow-2xl rounded-[45px] relative text-left">
          
          <nav className="h-24 px-12 flex items-center justify-between border-b border-slate-200 bg-white/80 backdrop-blur-md z-10 flex-shrink-0 text-left">
          ...
          <div className="flex-1 overflow-hidden flex bg-[#f8fafc]/30 text-left">
            {loading ? (
              ...
              <div className="flex-1 overflow-y-auto internal-scroll p-12 scrollbar-hide text-left">
                <div className="max-w-6xl mx-auto space-y-20">
                  {Object.keys(groupedRooms).map((dormId) => (
                    <div key={dormId} className="space-y-8 text-left">
                      <div className="flex items-center gap-6 text-left">
                        <div className="flex items-center gap-3">
                          <div className="w-2.5 h-2.5 rounded-full bg-emerald-500 shadow-[0_0_12px_rgba(16,185,129,0.4)]" />
                          <h2 className="text-lg font-black uppercase tracking-[0.4em] text-[#022c22] italic text-left">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-12 text-left">
                        {groupedRooms[dormId].map((room: any) => (
                          <motion.div 
                            ...
                            className={`bg-white border-2 p-8 rounded-[50px] shadow-sm cursor-pointer transition-all relative min-h-[350px] flex flex-col justify-between ${
                          ...
                          >
                            <div className="flex justify-between items-start text-left">
                               ...
                               <div className="p-3 bg-slate-50 rounded-2xl shadow-inner text-left"><LayoutGrid size={18} className="text-slate-300" /></div>
                            </div>

                            {/* FIXED GRID GRID FOR AVATARS - PARA HINDI SABOG */}
                            <div className="grid grid-cols-4 gap-4 my-8 w-full place-items-center">
                               {[...Array(room.total)].map((_, i) => {
                                 const resident = room.occupants[i];
                                 return (
                                   <div key={i} className="relative w-11 h-11">
                                      <motion.div 
                                        layoutId={resident ? `res-${resident.id}` : `empty-${room.id}-${i}`}
                                        className={`h-11 w-11 rounded-full border-2 flex items-center justify-center transition-all ${
                                          resident ? 'bg-emerald-950 border-emerald-800 shadow-lg' : 'border-slate-300 border-dashed bg-slate-100/50'
                                        }`}
                                      >
                            ...
                            <div className="flex items-center justify-between pt-5 border-t border-slate-50 text-left">
                               <div className="flex items-center gap-2 px-4 py-1.5 bg-slate-50 rounded-full border border-slate-100 text-left">
            ...
            <AnimatePresence>
              {selectedRoom && (
                <motion.aside initial={{ x: 450 }} animate={{ x: 0 }} exit={{ x: 450 }} className="w-[450px] bg-white border-l border-slate-300 shadow-2xl z-20 flex flex-col text-left">
                  <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-white/80 backdrop-blur-md text-left text-slate-900">
                     <h2 className="text-[11px] font-black text-slate-900 uppercase tracking-[0.4em] italic text-left">Room Intel</h2>
                     <button onClick={() => {setSelectedRoom(null); setMovingResident(null);}} className="p-2 hover:bg-slate-50 rounded-xl text-slate-400 transition-colors"><X size={20} /></button>
                  </div>

                  <div className="p-8 flex-1 overflow-y-auto space-y-8 text-left scrollbar-hide bg-[#f8fafc]/50 text-slate-900">
                      {!movingResident ? (
                        <>
                          <div className="bg-emerald-950 p-8 rounded-[40px] text-white shadow-2xl relative overflow-hidden border border-emerald-800 text-left">
                            <Sparkles className="absolute top-4 right-4 text-emerald-500/20" size={40} />
                            <p className="text-[10px] font-black uppercase tracking-widest opacity-60 italic text-left">{selectedRoom.floor}</p>
                            <h3 className="text-5xl font-extralight italic tracking-tighter leading-none mt-2 uppercase text-left">Room {selectedRoom.displayId}</h3>
                          </div>

                          <div className="space-y-5 text-left">
                            <p className="text-[10px] font-black text-slate-900 uppercase tracking-widest border-l-2 border-emerald-500 pl-3 italic text-left">Registered Occupants</p>
                            <div className="space-y-3 text-left">
                               {selectedRoom.occupants.map((occ: any) => (
                                 <div key={occ.id} className="flex items-center justify-between p-5 bg-white border border-slate-100 rounded-[30px] shadow-sm hover:border-emerald-200 transition-all text-left">
                                    <div className="flex items-center gap-5 text-left">
                                       ...
                                       <div className="text-left">
                                          <p className="text-sm font-bold text-slate-800 leading-none mb-1.5 uppercase text-left">{occ.name}</p>
                                          {/* UPDATED: COURSE AND YEAR LEVEL */}
                                          <p className="text-[9px] text-slate-400 font-black uppercase tracking-widest italic leading-none text-left">{occ.course} • {occ.year}</p>
                                       </div>
                                    </div>
                        ...
                        <div className="space-y-8 animate-in slide-in-from-right duration-300 text-left text-slate-900">
                           <div className="flex items-center gap-4 bg-blue-50 p-6 rounded-[35px] border border-blue-100 text-left">
                              <div className="w-12 h-12 rounded-2xl bg-blue-600 text-white flex items-center justify-center font-black italic shadow-lg text-left">{movingResident.initial}</div>
                              <div className="text-left">
                                 <p className="text-[9px] font-black text-blue-600 uppercase tracking-widest mb-1 italic text-left">Relocation logic</p>
                                 <h3 className="text-lg font-bold text-slate-800 uppercase text-left">{movingResident.name}</h3>
                              </div>
                              <button onClick={() => setMovingResident(null)} className="ml-auto text-slate-400 hover:text-slate-900"><X size={16}/></button>
                           </div>

                           <p className="text-[10px] font-black text-slate-900 uppercase tracking-widest border-l-2 border-blue-500 pl-3 italic text-left">Select Destination Unit</p>
                              ...
                              {rooms.map((r) => {
                                ...
                                return (
                                  ...
                                  >
                                     ...
                                     <p className="text-[9px] font-bold text-slate-400">{r.occupants.length}/{r.total}</p>
                  ...
                  <div className="p-8 border-t border-slate-100 mt-auto bg-white/80 backdrop-blur-md">
                      {movingResident ? (
                        ...
                        <button onClick={() => router.push('/manager/residents')} className="w-full bg-emerald-950 text-white py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] shadow-xl hover:scale-[1.02] transition-all">VIEW MASTERLIST</button>
                      )}
                  </div>
                </motion.aside>
              )}
