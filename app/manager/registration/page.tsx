...
export default function RegistrationPage() {
  ...
  // States para sa active period
  const [activeYear, setActiveYear] = useState<any>(null);
  const [activeSem, setActiveSem] = useState<any>(null);

  // States para sa Archive Search
      ...
      onAuthStateChanged(auth, async (user) => {
        if (user) {
          ...
          if (userSnap.exists()) {
            ...
            // Fetch Archived Residents for this building
            const archQ = query(
          ...
          const yearQ = query(collection(db, "academic_years"), where("status", "==", "active"));
          const yearSnap = await getDocs(yearQ);
          if (!yearSnap.empty) setActiveYear({ id: yearSnap.docs[0].id, ...yearSnap.docs[0].data() });

          const semQ = query(collection(db, "semesters"), where("status", "==", "active"));
          const semSnap = await getDocs(semQ);
          if (!semSnap.empty) setActiveSem({ id: semSnap.docs[0].id, ...semSnap.docs[0].data() });
        }
      });
    };
    fetchContext();
  ...
  const handleSelectArchived = (student: any) => {
    ...
    alert(`Data for ${student.first_name} has been restored to the form. ✨`);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await setDoc(residentRef, {
        ...
        registered_academic_year_id: activeYear.id,
        registered_academic_year_label: activeYear.label,
        registered_semester_id: activeSem.id,
  ...
  return (
    <div className="flex h-screen w-screen bg-[#f1f5f9] p-6 gap-6 overflow-hidden text-left font-sans text-slate-900 leading-none">
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-white border border-slate-300 shadow-[0_0_50px_rgba(0,0,0,0.08)] rounded-[45px] relative">
        ...
        <nav className="h-24 px-12 flex items-center justify-between border-b border-slate-200 bg-white/80 backdrop-blur-md relative z-10 flex-shrink-0">
          <div className="space-y-0.5 text-left">
            <div className="flex items-center gap-2">
               ...
               <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] leading-none italic">Registration Hub</p>
            </div>
          ...
          <div className="max-w-[1400px] mx-auto space-y-10">
            <div className="grid grid-cols-12 gap-8 items-stretch">
              <div className="col-span-8 space-y-8 flex flex-col">
                <div className="bg-white border border-slate-300 p-8 rounded-[40px] shadow-sm relative overflow-hidden flex-1 text-left leading-none">
                  <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-3 text-left">
                      ...
                      <h2 className="text-[11px] font-black uppercase tracking-[0.3em] italic">01. Academic Identity</h2>
                    </div>
                    <button onClick={handleIdLookup} disabled={searching} className="text-[10px] font-bold text-blue-500 hover:underline flex items-center gap-1 uppercase tracking-widest">
                ...
                <div className="bg-white border border-slate-300 p-8 rounded-[40px] shadow-sm text-left leading-none">
                  <div className="flex items-center gap-3 mb-8">
                    ...
                    <h2 className="text-[11px] font-black uppercase tracking-[0.3em] italic">02. Connectivity Hub</h2>
                  </div>
              ...
              <div className="col-span-4 text-left leading-none">
                <div className="bg-white border border-slate-300 p-8 rounded-[40px] shadow-sm h-full flex flex-col text-left">
            ...
            <div className="bg-white border border-slate-300 p-10 rounded-[45px] shadow-sm relative overflow-hidden text-left leading-none">
               ...
               <div className="grid grid-cols-3 gap-12 text-left leading-none">
                  <div className="space-y-4 border-r border-slate-100 pr-8 text-left leading-none">
                     <p className="text-[10px] font-black text-slate-900 uppercase tracking-widest pl-2 border-l-2 border-emerald-500 italic">Maternal</p>
                  ...
                  <div className="space-y-4 border-r border-slate-100 pr-8 text-left leading-none">
                     <p className="text-[10px] font-black text-slate-900 uppercase tracking-widest pl-2 border-l-2 border-blue-500 italic">Paternal</p>
                  ...
                  <div className="space-y-4 text-left leading-none">
                     <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2 border-l-2 border-slate-300 italic">Guardian</p>
        ...
        <div className="p-8 border-t border-slate-200 bg-white/80 backdrop-blur-md flex justify-end items-center gap-8 relative z-10 px-14 flex-shrink-0 text-left leading-none">
          <button type="button" onClick={() => setFormData(initialFormState)} className="px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-rose-600 transition-all">Clear Form</button>
          <button type="button" onClick={handleSubmit} disabled={loading} className="flex items-center gap-3 bg-emerald-950 text-white px-14 py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] shadow-2xl hover:scale-[1.02] active:scale-95 transition-all">
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
              ...
              <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative bg-[#f8fafc] w-full max-w-2xl rounded-[45px] shadow-2xl overflow-hidden border border-white flex flex-col max-h-[80vh]">
                <div className="p-8 border-b border-slate-200 bg-white flex justify-between items-center text-left leading-none">
                  <div>
                    <h2 className="text-xl font-black italic uppercase tracking-tight text-slate-900">Archive Retrieval</h2>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Previous residents of this building</p>
                  </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-2 internal-scroll text-left leading-none">
                  {filteredArchives.length === 0 ? (
                    <div className="py-20 text-center text-slate-300 uppercase text-[10px] font-black italic tracking-widest">No archived records match</div>
                  ) : (
                    filteredArchives.map((res) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3 text-slate-300 group-hover:text-emerald-500 transition-all leading-none">
  ...
  return (
    <div className="space-y-2 text-left leading-none">
      ...
      <div className="flex items-center gap-3 bg-white border-2 border-slate-200 px-5 py-3.5 rounded-2xl focus-within:border-emerald-500 transition-all shadow-sm group leading-none">
        {icon && <span className="text-slate-300 group-focus-within:text-emerald-500 transition-colors shrink-0">{icon}</span>}
  ...
  return (
    <div className="space-y-2 text-left leading-none">
      ...
      <div className="flex items-center gap-3 bg-white border-2 border-slate-200 px-5 py-3.5 rounded-2xl focus-within:border-emerald-500 transition-all shadow-sm leading-none">
        <select name={name} value={value} onChange={onChange} className="bg-transparent text-xs font-bold text-slate-700 outline-none w-full appearance-none leading-none">
