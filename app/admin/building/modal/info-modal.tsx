...
export default function DormInfoModal({ isOpen, onClose, dormData }: any) {
  ...
  const [assignedManager, setAssignedManager] = useState<any>(null); // State para sa manager
  const [loading, setLoading] = useState(false);
  const router = useRouter();

  useEffect(() => {
    if (isOpen && dormData) {
      // 1. FETCH ASSIGNED MANAGER BASE SA BUILDING GROUP
      const fetchManager = async () => {
        const q = query(
          collection(db, "users"), 
          where("role", "==", "dorm_manager"),
          where("assigned_building", "==", dormData.group || dormData.title),
          limit(1)
        );
      ...
      // 2. FETCH RESIDENTS (TAB SPECIFIC)
      if (activeTab === 'residents') {
        setLoading(true);
        const q = query(
          collection(db, "allocations"), 
          where("dormID", "==", dormData.id || dormData.title.replace(/\s+/g, '').toUpperCase())
        );

        const unsub = onSnapshot(q, async (snap) => {
          const residentsList: any[] = [];
          for (const allocationDoc of snap.docs) {
            const allocData = allocationDoc.data();
            const resDoc = await getDoc(doc(db, "residents", allocData.studentID));
            if (resDoc.exists()) {
              residentsList.push({
                id: allocationDoc.id,
                room: allocData.roomID.split('-').pop(),
                ...resDoc.data()
              });
            }
          }
  ...
  const handleOpenFloorPlan = () => {
    const slug = dormData.title.toLowerCase().replace(/ /g, '-').replace(/&/g, 'and');
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0, y: 60 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 60 }} className="relative w-full max-w-5xl bg-[#fcfdfe] rounded-[56px] shadow-2xl overflow-hidden flex h-[680px] border border-white/50">
            ...
            <div className="relative z-10">
              <div className="space-y-2 mb-8 text-left">
                ...
                <div className="h-2 w-full bg-white/10 rounded-full overflow-hidden p-[2px]">
                   <motion.div initial={{ width: 0 }} animate={{ width: `${percent}%` }} className="h-full bg-emerald-400 rounded-full shadow-[0_0_20px_#10b981]" />
                </div>
              </div>
          ...
          <div className="flex-1 flex flex-col bg-[#F8FAFC] overflow-hidden">
            <div className="px-14 pt-12 pb-6 flex items-center justify-between border-b border-slate-100 flex-shrink-0 bg-white">
               ...
               <button onClick={onClose} className="p-3 hover:bg-slate-50 rounded-full text-slate-300 hover:text-slate-900 border border-slate-100"><X size={18} /></button>
            </div>
                 ...
                 <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-12">
                    ...
                    <section className="text-left">
                      ...
                      <div className="grid grid-cols-2 gap-5">
                        ...
                        <InfoItem icon={<Activity size={18} />} label="Registry" value="Live" />
                      </div>
                    </section>
                 </motion.div>
               )}
               ...
               {activeTab === 'residents' && (
                 <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                    {loading ? (
                      ...
                      <div key={r.id} className="p-5 bg-white rounded-3xl border border-slate-100 shadow-sm flex items-center justify-between hover:border-emerald-200 transition-all text-left">
                        <div className="flex items-center gap-4 text-left">
                          <div className="w-12 h-12 rounded-xl bg-slate-50 flex items-center justify-center text-emerald-700 font-bold text-xs uppercase">{r.room}</div>
                          <div className="text-left">
                            <p className="text-sm font-semibold text-slate-800 tracking-tight uppercase">{r.first_name} {r.last_name}</p>
                            <p className="text-[10px] text-slate-400 uppercase tracking-widest">{r.course} • {r.yearLevel}</p>
                          </div>
                        </div>
                      ...
                      <p className="text-center text-slate-400 py-20 text-xs uppercase tracking-widest">No residents detected.</p>
                    )}
                 </motion.div>
               )}
            </div>

            <div className="px-14 py-8 bg-white border-t border-slate-50 flex gap-4 flex-shrink-0">
              ...
              <button className="px-10 py-5 rounded-[22px] border border-slate-200 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 hover:bg-slate-50 transition-all">Archive</button>
            </div>
          </div>
        </motion.div>
      </div>
...
function InfoItem({ icon, label, value }: any) {
  return (
    <div className="p-7 rounded-[35px] border border-slate-100 bg-white shadow-sm hover:shadow-lg transition-all duration-500 group text-left">
      <div className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-emerald-700 mb-5 group-hover:scale-110 transition-transform">{icon}</div>
      <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1 text-left">{label}</p>
      <p className="text-sm font-semibold text-slate-800 tracking-tight leading-none text-left">{value}</p>
    </div>
  );
}